...
Sarah Hennessy & Priscilla Perez
January 19, 2024
B&M Workshop Series
...
# factor
fac = factor(c("cat","cat","dog","mouse","dog"))
...
# dataframe
animal <- factor(c("cat","cat","dog","mouse","dog"))
adoptee_number <- c(1:5)
...
```{r}
myplot <- ggplot(data, aes(x = age_group, y = memory)) + 
  geom_boxplot()
...
```{r}
myplot <- ggplot(data, aes(x = age_group, y = memory, color = condition)) + 
  geom_boxplot()
...
need to use wide again..but this time we need it wide where it is spread by age group...
...
mean(data_wide_age$memory.Younger,na.rm=TRUE)
sd(data_wide_age$memory.Younger,na.rm=TRUE)
```
...
```{r}
mean(data_wide$Nostalgia_memory,na.rm=TRUE)
sd(data_wide$Nostalgia_memory,na.rm=TRUE)
mean(data_wide$Control_memory,na.rm=TRUE)
sd(data_wide$Control_memory,na.rm=TRUE)
```
...
main effects and iteractions and a covariate
...
Do a post of to look at pairwise
